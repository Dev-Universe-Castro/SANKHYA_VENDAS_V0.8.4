...
export async function obterTokenAtual(): Promise<TokenStatus | null> {
  try {
    ...
    console.error('❌ Erro ao obter token atual:', erro);
    return null;
  }
}
...
export async function obterToken(forceRefresh = false, retryCount = 0): Promise<string> {
  ...
  tokenPromise = (async () => {
    ...
    cachedToken = token;
      ...
      await redisCacheService.set('sankhya:token', tokenData, 20 * 60 * 1000); // 20 minutos
      console.log("💾 Token salvo no Redis:", { geradoEm, expiresAt: new Date(expiresAt).toISOString() });
